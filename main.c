...

{
    ...
    Graphics_drawStringCentered(&g_sContext, "ECE2049-C24!", AUTO_STRING_LENGTH, 48, 35, TRANSPARENT_TEXT);
    Graphics_drawStringCentered(&g_sContext, name , AUTO_STRING_LENGTH, 48, 45, TRANSPARENT_TEXT);
    Graphics_drawStringCentered(&g_sContext, "Space Trip", AUTO_STRING_LENGTH, 48, 55, TRANSPARENT_TEXT);
    ...
    while (1)    // Forever loop
    {
        ...
        if (currKey == '*')
            ...
            BuzzerOff();
        else if ((currKey >= '0') && (currKey <= '9'))
            setLeds(currKey - 0x30);

        if (currKey)
        {
            // Clear previous character from the display
            Graphics_clearDisplay(&g_sContext);

            // Create string representation of the key
            char dispStr[2] = {currKey, '\0'}; // Convert current key to string

            // Draw the new character to the display
            Graphics_drawStringCentered(&g_sContext, (uint8_t *)dispStr, AUTO_STRING_LENGTH, 48, 55, OPAQUE_TEXT);
